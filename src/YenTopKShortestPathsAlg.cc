...
void YenTopKShortestPathsAlg::_init()
{
	...
	{
		shared_ptr<BasePath> pShortestPath(get_shortest_path(m_pSourceVertex, m_pTargetVertex));
...
shared_ptr<BasePath> YenTopKShortestPathsAlg::get_shortest_path( shared_ptr<BaseVertex> pSource, shared_ptr<BaseVertex> pTarget )
{
...
shared_ptr<BasePath> YenTopKShortestPathsAlg::next()
{
	//1. Prepare for removing vertices and arcs
	shared_ptr<BasePath> cur_path = *(m_quPathCandidates.begin());//m_quPathCandidates.top();
	...
	shared_ptr<BaseVertex> cur_derivation_pt = m_mpDerivationVertexIndex.find(cur_path)->second; 
	vector<shared_ptr<BaseVertex>> sub_path_of_derivation_pt;
	...
	{
		shared_ptr<BasePath> cur_result_path = m_vResultList.at(i);
		vector<shared_ptr<BaseVertex>> cur_result_sub_path_of_derivation_pt;
		...
		//
		shared_ptr<BaseVertex> cur_succ_vertex = cur_result_path->GetVertex(sub_path_length+1);
	...
	for(int i=path_length-2; i>=0 && !is_done; --i)
	{
		...
		shared_ptr<BaseVertex> cur_recover_vertex = cur_path->GetVertex(i);
		...
		shared_ptr<BasePath> sub_path = reverse_tree.update_cost_forward(cur_recover_vertex);
		...
		if (sub_path != NULL)
		{
			...
			vector<shared_ptr<BaseVertex>> pre_path_list;
			for (int j=0; j<path_length; ++j)
			{
				shared_ptr<BaseVertex> cur_vertex = cur_path->GetVertex(j);
			...
			sub_path = shared_ptr<BasePath>(new Path(pre_path_list, cost+sub_path->Weight()));
		...
		shared_ptr<BaseVertex> succ_vertex = cur_path->GetVertex(i+1);
...
void YenTopKShortestPathsAlg::get_shortest_paths( shared_ptr<BaseVertex> pSource, 
	shared_ptr<BaseVertex> pTarget, int top_k, vector<shared_ptr<BasePath>>& result_list)
{
...
void YenTopKShortestPathsAlg::SetNextCompute(shared_ptr<BaseVertex> pSource, shared_ptr<BaseVertex> pTarget){
	m_pSourceVertex = pSource;
	m_pTargetVertex = pTarget;
	clear();
	if (m_pSourceVertex != NULL && m_pTargetVertex != NULL)
