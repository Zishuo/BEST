...
shared_ptr<BasePath> DijkstraShortestPathAlg::get_shortest_path( shared_ptr<BaseVertex> source, shared_ptr<BaseVertex> sink ) {
    determine_shortest_paths(source, sink, true);

    std::vector<shared_ptr<BaseVertex>> vertex_list;
    std::map<shared_ptr<BaseVertex>, double>::const_iterator pos =
    ...
    if (weight < Graph::DISCONNECT) {
        shared_ptr<BaseVertex> cur_vertex_pt = sink;
        do {
            vertex_list.insert(vertex_list.begin(), cur_vertex_pt);

            std::map<shared_ptr<BaseVertex>, shared_ptr<BaseVertex>>::const_iterator pre_pos =
    ...
    return shared_ptr<BasePath>(new BasePath(vertex_list, weight));
}

void DijkstraShortestPathAlg::determine_shortest_paths( shared_ptr<BaseVertex> source, shared_ptr<BaseVertex> sink, bool is_source2sink ) {
    //1. clear the intermediate variables
    clear();

    //2. initiate the local variables
    shared_ptr<BaseVertex> end_vertex = is_source2sink ? sink : source;
    shared_ptr<BaseVertex> start_vertex = is_source2sink ? source : sink;
    ...
    while (!m_quCandidateVertices.empty()) {
        multiset<shared_ptr<BaseVertex>, WeightLess<shared_ptr<BaseVertex> > >::const_iterator pos = m_quCandidateVertices.begin();

        shared_ptr<BaseVertex> cur_vertex_pt = *pos; //m_quCandidateVertices.top();
...
void DijkstraShortestPathAlg::improve2vertex( shared_ptr<BaseVertex> cur_vertex_pt, bool is_source2sink ) {
    // 1. get the neighboring vertices
    shared_ptr<set<shared_ptr<BaseVertex>>> neighbor_vertex_list_pt(new set<shared_ptr<BaseVertex>>());
    ...
    for(set<shared_ptr<BaseVertex>>::iterator cur_neighbor_pos=neighbor_vertex_list_pt->begin();
        ...
        map<shared_ptr<BaseVertex>, double>::const_iterator cur_pos = m_mpStartDistanceIndex.find(cur_vertex_pt);
...
shared_ptr<BasePath> DijkstraShortestPathAlg::update_cost_forward( shared_ptr<BaseVertex> vertex ) {
    double cost = Graph::DISCONNECT;

    // 1. get the set of successors of the input vertex
    shared_ptr<set<shared_ptr<BaseVertex>>> adj_vertex_set(new set<shared_ptr<BaseVertex>>());
    m_pDirectGraph->get_adjacent_vertices(vertex, *adj_vertex_set);

    // 2. make sure the input vertex exists in the index
    map<shared_ptr<BaseVertex>, double>::iterator pos4vertexInStartDistIndex = m_mpStartDistanceIndex.find(vertex);
    ...
    for(set<shared_ptr<BaseVertex>>::const_iterator pos=adj_vertex_set->begin(); pos!=adj_vertex_set->end(); ++pos) {
        // 3.1 get the distance from the root to one successor of the input vertex
        map<shared_ptr<BaseVertex>, double>::const_iterator cur_vertex_pos = m_mpStartDistanceIndex.find(*pos);
    ...
    shared_ptr<BasePath> sub_path = NULL;
    if(cost < Graph::DISCONNECT) {
        vector<shared_ptr<BaseVertex>> vertex_list;
        vertex_list.push_back(vertex);

        map<shared_ptr<BaseVertex>, shared_ptr<BaseVertex>>::const_iterator pos4PredVertexMap =
                    m_mpPredecessorVertex.find(vertex);

        while(pos4PredVertexMap != m_mpPredecessorVertex.end()) {
            shared_ptr<BaseVertex> pred_vertex_pt = pos4PredVertexMap->second;
            vertex_list.push_back(pred_vertex_pt);
            pos4PredVertexMap = m_mpPredecessorVertex.find(pred_vertex_pt);
        }

        sub_path = shared_ptr<BasePath>(new BasePath(vertex_list, cost));
    }
    return sub_path;
}

void DijkstraShortestPathAlg::correct_cost_backward( shared_ptr<BaseVertex> vertex ) {
    // 1. initialize the list of vertex to be updated
    vector<shared_ptr<BaseVertex>> vertex_pt_list;
    vertex_pt_list.push_back(vertex);

    // 2. update the cost of relevant precedents of the input vertex
    while(!vertex_pt_list.empty()) {
        shared_ptr<BaseVertex> cur_vertex_pt = *(vertex_pt_list.begin());
        vertex_pt_list.erase(vertex_pt_list.begin());

        double cost_of_cur_vertex = m_mpStartDistanceIndex[cur_vertex_pt];

        set<shared_ptr<BaseVertex>> pre_vertex_set;
        m_pDirectGraph->get_precedent_vertices(cur_vertex_pt, pre_vertex_set);
        for(set<shared_ptr<BaseVertex>>::const_iterator pos=pre_vertex_set.begin(); pos!=pre_vertex_set.end(); ++pos) {
            map<shared_ptr<BaseVertex>,double>::const_iterator pos4StartDistIndexMap =
